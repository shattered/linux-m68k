...
static int slm_read( struct inode *node, struct file* file, char *buf, int
                     count );
...
static int slm_write( struct inode *node, struct file *file, const char *buf, int
                      count );
...
static int slm_read( struct inode *node, struct file* file,
					 char *buf, int count )
...
static int slm_write( struct inode *node, struct file *file,
...
{	int		device = MINOR( inode->i_rdev ), err;
	...
	switch( cmd ) {
	  ...
	  case SLMIOGSTAT: {	/* get status */
		...
		if (arg) {
			err = verify_area( VERIFY_WRITE, (long *)arg,
							   sizeof(struct SLM_status) );
			if (err) return( err );

			str = slm_errstr( stat );
			put_fs_long( stat, &((struct SLM_status *)arg)->stat );
			memcpy_tofs( ((struct SLM_status *)arg)->str, str,
						 strlen(str) + 1 );
		}
		return( stat );
	  }
	  ...
	  case SLMIOGPSIZE: {	/* get paper size */
		...
		put_fs_long( w, &((struct SLM_paper_size *)arg)->width );
		put_fs_long( h, &((struct SLM_paper_size *)arg)->height );
		return( 0 );
	  }
