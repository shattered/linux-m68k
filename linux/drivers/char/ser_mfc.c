 ...
 * Test it, and let me know if it works. Changing the baud table isn't worth
 * whole new driver.
 ...
 * The only one who knows that now effectivly all baudrates are doubled are you
 * and not any application. Besides, you should know what you are doing and you do it
 * at your own risk.)
 *
 ...
 * But that would mean to support only standart baud rates or to get into
 * big difficulties. On the other hand, you had more standard rates available.
 *
...
static void mfc_init( struct async_struct *info );
static void mfc_deinit( struct async_struct *info, int leave_dtr );
static void mfc_enab_tx_int( struct async_struct *info, int enab_flag );
static int  mfc_check_custom_divisor( struct async_struct *info, int
				     baud_base, int divisor );
static void mfc_change_speed( struct async_struct *info );
static void mfc_throttle( struct async_struct *info, int status );
static void mfc_set_break( struct async_struct *info, int break_flag );
static void mfc_get_serial_info( struct async_struct *info,
				struct serial_struct *retinfo );
static unsigned int mfc_get_modem_info( struct async_struct *info );
static int mfc_set_modem_info( struct async_struct *info, int new_dtr,
			      int new_rts );
static void mfc_stop_receive(struct async_struct *info);
static int mfc_trans_empty(struct async_struct *info);
...
static void mfc_fill_tx(struct async_struct *info)
{
struct duart *dp;
struct duarthalf *d;
int sta,i,mask,ch;
...
static int mfc_check_custom_divisor(struct async_struct *info,
...
static int mfc_set_modem_info(struct async_struct *info, int new_dtr, int new_rts)
{
struct duart *dp;
