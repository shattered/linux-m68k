			      ...
			      unsigned long bitmap_nr)
{
	...
	bh = bread (sb->s_dev, e_swab (bs, gdp->bg_block_bitmap),
		    sb->s_blocksize);
	if (!bh) {
		ext2_error (sb, "read_block_bitmap",
			    ...
			    block_group,
			    (unsigned long) e_swab (bs, gdp->bg_block_bitmap));
		retval = -EIO;
	}
	/*
		       ...
		       unsigned long count)
{
	...
	if (block < e_swab (bs, es->s_first_data_block) || 
	    (block + count) > e_swab (bs, es->s_blocks_count)) {
	...
	block_group = (block - e_swab (bs, es->s_first_data_block)) /
		      EXT2_BLOCKS_PER_GROUP(sb);
	bit = (block - e_swab (bs, es->s_first_data_block)) %
	      EXT2_BLOCKS_PER_GROUP(sb);
	...
	if (test_opt (sb, CHECK_STRICT) &&
	    (in_range (e_swab (bs, gdp->bg_block_bitmap), block, count) ||
	     in_range (e_swab (bs, gdp->bg_inode_bitmap), block, count) ||
	     in_range (block, e_swab (bs, gdp->bg_inode_table),
		       sb->u.ext2_sb.s_itb_per_group) ||
	     in_range (block + count - 1, e_swab (bs, gdp->bg_inode_table),
		       sb->u.ext2_sb.s_itb_per_group))) {
		ext2_error (sb, "ext2_free_blocks",
	...
	for (i = 0; i < count; i++) {
		if (!ext2_clear_bit (bit + i, bh->b_data))
			...
			e_set_swab (bs, gdp->bg_free_blocks_count,
				    e_swab (bs, gdp->bg_free_blocks_count) + 1);
			e_set_swab (bs, es->s_free_blocks_count,
				    e_swab (bs, es->s_free_blocks_count) + 1);
		}
	}
		    ...
		    u32 * prealloc_count, u32 * prealloc_block, int * err)
{
	...
	*err = -ENOSPC;
	...
	bs = BYTE_SWAP(sb->u.ext2_sb.s_byte_swapped);
	es = sb->u.ext2_sb.s_es;
	if (e_swab (bs, es->s_free_blocks_count) <= e_swab (bs, es->s_r_blocks_count) &&
...
repeat:
	 ...
	 */
	if (goal < e_swab (bs, es->s_first_data_block) ||
	    goal >= e_swab (bs, es->s_blocks_count))
		goal = e_swab (bs, es->s_first_data_block);
	i = (goal - e_swab (bs, es->s_first_data_block)) /
	    EXT2_BLOCKS_PER_GROUP(sb);
	gdp = get_group_desc (sb, i, &bh2);
	if (e_swab (bs, gdp->bg_free_blocks_count) > 0) {
		j = ((goal - e_swab (bs, es->s_first_data_block)) %
		    EXT2_BLOCKS_PER_GROUP(sb));
#ifdef EXT2FS_DEBUG
		if (j)
			goal_attempts++;
...
got_block:
	...
	tmp = j + i * EXT2_BLOCKS_PER_GROUP(sb) +
	      e_swab (bs, es->s_first_data_block);

	if (test_opt (sb, CHECK_STRICT) &&
	    (tmp == e_swab (bs, gdp->bg_block_bitmap) ||
	     tmp == e_swab (bs, gdp->bg_inode_bitmap) ||
	     in_range (tmp, e_swab (bs, gdp->bg_inode_table), sb->u.ext2_sb.s_itb_per_group))) {
		ext2_error (sb, "ext2_new_block",
	...
	if (prealloc_block) {
		...
		*prealloc_block = tmp + 1;
		...
		e_set_swab (bs, gdp->bg_free_blocks_count,
			    e_swab (bs, gdp->bg_free_blocks_count) - *prealloc_count);
		e_set_swab (bs, es->s_free_blocks_count,
			    e_swab (bs, es->s_free_blocks_count) - *prealloc_count);
	...
	if (j >= e_swab (bs, es->s_blocks_count)) {
		ext2_error (sb, "ext2_new_block",
			    "block >= blocks count - "
...
unsigned long ext2_count_free_blocks (struct super_block * sb)
{
	...
	for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
		...
		desc_count += e_swab (bs, gdp->bg_free_blocks_count);
		...
		printk ("group %d: stored = %d, counted = %lu\n",
			i, e_swab (bs, gdp->bg_free_blocks_count), x);
		bitmap_count += x;
	}
	printk("ext2_count_free_blocks: stored = %lu, computed = %lu, %lu\n",
	       e_swab (bs, es->s_free_blocks_count), desc_count, bitmap_count);
	unlock_super (sb);
	return bitmap_count;
#else
...
static inline int block_in_use (unsigned long block,
				...
				unsigned char * map)
{
	return ext2_test_bit ((block - e_swab (BYTE_SWAP(sb->u.ext2_sb.s_byte_swapped),
 	                 sb->u.ext2_sb.s_es->s_first_data_block)) %
...
void ext2_check_blocks_bitmap (struct super_block * sb)
{
	...
	for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
		...
		if (!block_in_use (e_swab (bs, gdp->bg_block_bitmap), sb, bh->b_data))
			ext2_error (sb, "ext2_check_blocks_bitmap",
				    "Block bitmap for group %d is marked free",
				    i);

		if (!block_in_use (e_swab (bs, gdp->bg_inode_bitmap), sb, bh->b_data))
			ext2_error (sb, "ext2_check_blocks_bitmap",
				    "Inode bitmap for group %d is marked free",
				    i);

		bg_inode_table = e_swab (bs, gdp->bg_inode_table);
		for (j = 0; j < sb->u.ext2_sb.s_itb_per_group; j++)
			if (!block_in_use (bg_inode_table + j, sb, bh->b_data))
				ext2_error (sb, "ext2_check_blocks_bitmap",
					    "Block #%d of the inode table in "
					    "group %d is marked free", j, i);

		x = ext2_count_free (bh, sb->s_blocksize);
		if (e_swab (bs, gdp->bg_free_blocks_count) != x)
			ext2_error (sb, "ext2_check_blocks_bitmap",
				    "Wrong free blocks count for group %d, "
				    "stored = %d, counted = %lu", i,
				    e_swab (bs, gdp->bg_free_blocks_count), x);
		bitmap_count += x;
	}
	if (e_swab (bs, es->s_free_blocks_count) != bitmap_count)
		ext2_error (sb, "ext2_check_blocks_bitmap",
			    "Wrong free blocks count in super block, "
			    "stored = %lu, counted = %lu",
			    (unsigned long) e_swab (bs, es->s_free_blocks_count),
			    bitmap_count);
	unlock_super (sb);
}
