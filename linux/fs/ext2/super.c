		 ...
		 const char * fmt, ...)
{
	...
	if (!(sb->s_flags & MS_RDONLY)) {
		...
		sb->u.ext2_sb.s_es->s_state |= (bs ? EXT2_ERROR_FS << 8 : EXT2_ERROR_FS);
	...
	if (test_opt (sb, ERRORS_PANIC) ||
	    (e_swab (bs, sb->u.ext2_sb.s_es->s_errors) == EXT2_ERRORS_PANIC &&
	...
	if (test_opt (sb, ERRORS_RO) ||
	    (e_swab (bs, sb->u.ext2_sb.s_es->s_errors) == EXT2_ERRORS_RO &&
			    ...
			    const char * fmt, ...)
{
	...
	if (!(sb->s_flags & MS_RDONLY)) {
		...
		sb->u.ext2_sb.s_es->s_state |=
		  (BYTE_SWAP(sb->u.ext2_sb.s_byte_swapped) ? EXT2_ERROR_FS << 8 : EXT2_ERROR_FS);
		mark_buffer_dirty(sb->u.ext2_sb.s_sbh, 1);
		sb->s_dirt = 1;
	}
	va_start (args, fmt);
...
void ext2_put_super (struct super_block * sb)
{
	...
	lock_super (sb);
	if (!(sb->s_flags & MS_RDONLY)) {
		e_set_swab (BYTE_SWAP(sb->u.ext2_sb.s_byte_swapped),
			    sb->u.ext2_sb.s_es->s_state,
			    sb->u.ext2_sb.s_mount_state);
		mark_buffer_dirty(sb->u.ext2_sb.s_sbh, 1);
	}
	sb->s_dev = 0;
	...
	if (!(sb->s_flags & MS_RDONLY)) {
		if (!(sb->u.ext2_sb.s_mount_state & EXT2_VALID_FS))
			...
			printk ("EXT2-fs warning: mounting fs with errors, "
				"running e2fsck is recommended\n");
		else if (e_swab (bs, es->s_max_mnt_count) >= 0 &&
		         e_swab (bs, es->s_mnt_count) >=
			 (unsigned short) e_swab (bs, es->s_max_mnt_count))
			printk ("EXT2-fs warning: maximal mount count reached, "
				"running e2fsck is recommended\n");
		else if (es->s_checkinterval &&
			(e_swab (bs, es->s_lastcheck) + e_swab (bs, es->s_checkinterval) <= CURRENT_TIME))
			printk ("EXT2-fs warning: checktime reached, "
				"running e2fsck is recommended\n");
		es->s_state &= ~(bs ? EXT2_VALID_FS << 8 : EXT2_VALID_FS);
		if (!es->s_max_mnt_count)
			e_set_swab (bs, es->s_max_mnt_count, EXT2_DFL_MAX_MNT_COUNT);
		e_set_swab (bs, es->s_mnt_count,
			    e_swab (bs, es->s_mnt_count) + 1);
		e_set_swab (bs, es->s_mtime, CURRENT_TIME);
		mark_buffer_dirty(sb->u.ext2_sb.s_sbh, 1);
		sb->s_dirt = 1;
...
static int ext2_check_descriptors (struct super_block * sb)
{
	...
	int desc_block = 0;
	int bs = BYTE_SWAP(sb->u.ext2_sb.s_byte_swapped);
	unsigned long block = e_swab (bs, sb->u.ext2_sb.s_es->s_first_data_block);
	...
	for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++)
	{
		...
		if (e_swab (bs, gdp->bg_block_bitmap) < block ||
		    e_swab (bs, gdp->bg_block_bitmap) >= block + EXT2_BLOCKS_PER_GROUP(sb))
		{
			ext2_error (sb, "ext2_check_descriptors",
				    "Block bitmap for group %d"
				    " not in group (block %lu)!",
				    i, (unsigned long) e_swab (bs, gdp->bg_block_bitmap));
			return 0;
		}
		if (e_swab (bs, gdp->bg_inode_bitmap) < block ||
		    e_swab (bs, gdp->bg_inode_bitmap) >= block + EXT2_BLOCKS_PER_GROUP(sb))
		{
			ext2_error (sb, "ext2_check_descriptors",
				    "Inode bitmap for group %d"
				    " not in group (block %lu)!",
				    i, (unsigned long) e_swab (bs, gdp->bg_inode_bitmap));
			return 0;
		}
		if (e_swab (bs, gdp->bg_inode_table) < block ||
		    e_swab (bs, gdp->bg_inode_table) + sb->u.ext2_sb.s_itb_per_group >=
		    block + EXT2_BLOCKS_PER_GROUP(sb))
		{
			ext2_error (sb, "ext2_check_descriptors",
				    "Inode table for group %d"
				    " not in group (block %lu)!",
				    i, (unsigned long) e_swab (bs, gdp->bg_inode_table));
			return 0;
		}
		block += EXT2_BLOCKS_PER_GROUP(sb);
		gdp++;
				      ...
				      int silent)
{
	...
	sb->u.ext2_sb.s_byte_swapped = bs = 0;

	if (sb->s_magic != EXT2_SUPER_MAGIC) {
		sb->u.ext2_sb.s_byte_swapped = bs = 1;
		sb->s_magic = e_swab (bs, es->s_magic);
	}
	...
	if (e_swab (bs, es->s_rev_level) > EXT2_GOOD_OLD_REV) {
		if (e_swab (bs, es->s_feature_incompat) & ~EXT2_FEATURE_INCOMPAT_SUPP) {
		...
		if (!(sb->s_flags & MS_RDONLY) &&
		    (e_swab (bs, es->s_feature_ro_compat) & ~EXT2_FEATURE_RO_COMPAT_SUPP)) {
	...
	sb->s_blocksize_bits = e_swab (bs, es->s_log_block_size) + 10;
	...
	if (sb->s_blocksize != BLOCK_SIZE && 
		...
		sb->u.ext2_sb.s_es = es;
		if (e_swab (bs, es->s_magic) != EXT2_SUPER_MAGIC) {
	...
	if (e_swab (bs, es->s_rev_level) == EXT2_GOOD_OLD_REV) {
		...
		sb->u.ext2_sb.s_inode_size = e_swab (bs, es->s_inode_size);
		sb->u.ext2_sb.s_first_ino = e_swab (bs, es->s_first_ino);
	...
	sb->u.ext2_sb.s_frag_size = EXT2_MIN_FRAG_SIZE <<
				   e_swab (bs, es->s_log_frag_size);
	if (sb->u.ext2_sb.s_frag_size)
		sb->u.ext2_sb.s_frags_per_block = sb->s_blocksize /
						  sb->u.ext2_sb.s_frag_size;
	else
		sb->s_magic = 0;
	sb->u.ext2_sb.s_blocks_per_group = e_swab (bs, es->s_blocks_per_group);
	sb->u.ext2_sb.s_frags_per_group = e_swab (bs, es->s_frags_per_group);
	sb->u.ext2_sb.s_inodes_per_group = e_swab (bs, es->s_inodes_per_group);
	...
	if (resuid != EXT2_DEF_RESUID)
		...
		sb->u.ext2_sb.s_resuid = e_swab (bs, es->s_def_resuid);
	if (resgid != EXT2_DEF_RESGID)
		sb->u.ext2_sb.s_resgid = resgid;
	else
		sb->u.ext2_sb.s_resgid = e_swab (bs, es->s_def_resgid);
	sb->u.ext2_sb.s_mount_state = e_swab (bs, es->s_state);
	...
	sb->u.ext2_sb.s_groups_count = (e_swab (bs, es->s_blocks_count) -
				        e_swab (bs, es->s_first_data_block) +
...
static void ext2_commit_super (struct super_block * sb,
			       struct ext2_super_block * es)
{
	e_set_swab (BYTE_SWAP(sb->u.ext2_sb.s_byte_swapped), es->s_wtime, CURRENT_TIME);
	mark_buffer_dirty(sb->u.ext2_sb.s_sbh, 1);
	sb->s_dirt = 0;
}
...
void ext2_write_super (struct super_block * sb)
{
	...
	if (!(sb->s_flags & MS_RDONLY)) {
		...
		if (e_swab (bs, es->s_state) & EXT2_VALID_FS) {
			es->s_state &= ~(bs ? EXT2_VALID_FS << 8 : EXT2_VALID_FS);
			e_set_swab (bs, es->s_mtime, CURRENT_TIME);
		}
...
int ext2_remount (struct super_block * sb, int * flags, char * data)
{
	...
	if (*flags & MS_RDONLY) {
		 ...
		 */
		e_set_swab (bs, es->s_state, sb->u.ext2_sb.s_mount_state);
		e_set_swab (bs, es->s_mtime, CURRENT_TIME);
		mark_buffer_dirty(sb->u.ext2_sb.s_sbh, 1);
		sb->s_dirt = 1;
		ext2_commit_super (sb, es);
...
void ext2_statfs (struct super_block * sb, struct statfs * buf, int bufsiz)
{
	...
	if (test_opt (sb, MINIX_DF))
		...
		overhead = e_swab (bs, sb->u.ext2_sb.s_es->s_first_data_block) +
			   sb->u.ext2_sb.s_groups_count * overhead_per_group;
	}

	tmp.f_type = EXT2_SUPER_MAGIC;
	tmp.f_bsize = sb->s_blocksize;
	tmp.f_blocks = e_swab (bs, sb->u.ext2_sb.s_es->s_blocks_count) - overhead;
	tmp.f_bfree = ext2_count_free_blocks (sb);
	tmp.f_bavail = tmp.f_bfree - e_swab (bs, sb->u.ext2_sb.s_es->s_r_blocks_count);
	if (tmp.f_bfree < e_swab (bs, sb->u.ext2_sb.s_es->s_r_blocks_count))
		tmp.f_bavail = 0;
	tmp.f_files = e_swab (bs, sb->u.ext2_sb.s_es->s_inodes_count);
