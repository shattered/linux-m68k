...
static int amiga_fb_get_fix(struct fb_fix_screeninfo *fix, int con, int fbidx);
static int amiga_fb_get_var(struct fb_var_screeninfo *var, int con, int fbidx);
static int amiga_fb_set_var(struct fb_var_screeninfo *var, int con, int fbidx);
...
static int amiga_fb_set_cmap(struct fb_cmap *cmap, int kspc, int con, int fbidx);
static int amiga_fb_ioctl(struct inode *inode, struct file *file, u_int cmd,
                          u_long arg, int con, int fbidx);
static int amiga_fb_get_monitorspec(struct fb_monitorspec *spec, int val, int fbidx);
static int amiga_fb_put_monitorspec(struct fb_monitorspec *spec, int val, int fbidx);
...
#if defined(CONFIG_FB_CYBER) || defined(CONFIG_FB_CV3D)	/* Cybervision 64+3D */
...
static struct fb_ops amiga_fb_ops = {
	...
	amiga_fb_get_monitorspec, amiga_fb_put_monitorspec
};
...
void amiga_video_setup(char *options, int *ints)
{
	...
	if (options && *options)
		if ((!strncmp(options, "cyber", 5) || (!strncmp(options, "virgefb:", 8))) && Cyber_probe()) {
			amifb_Cyber = 1;
